...
type StaticEnvironment struct {
	...
	AuthBootstrapJS string `validate:"required,min=1"`
	APIBase         string `validate:"required,min=1"`
	SessionPath     string `validate:"required,min=1"`
	LoginURL        string `validate:"required,min=1"`
	AppJS           string `validate:"required,min=1"`
