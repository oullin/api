...
func (s *Sections) Profile(profileResponse *profile.ProfileResponse) template.HTML {
	...
	return "<h1>Profile</h1>" +
		template.HTML("<p>"+
			template.HTMLEscapeString(profileResponse.Data.Name)+", "+
			template.HTMLEscapeString(profileResponse.Data.Profession)+
			"</p>",
		)
}

func (s *Sections) Skills(profileResponse *profile.ProfileResponse) template.HTML {
	if profileResponse == nil {
		return template.HTML("")
	}
	...
	for _, item := range profileResponse.Data.Skills {
...
func (s *Sections) Talks(talksResponse *talks.TalksResponse) template.HTML {
	if talksResponse == nil {
		return template.HTML("")
	}

	var items []string

	for _, item := range talksResponse.Data {
...
func (s *Sections) Projects(projectsResponse *projects.ProjectsResponse) template.HTML {
	var items []string

	for _, item := range projectsResponse.Data {
		...
		href := portal.SanitiseURL(strings.TrimSpace(item.URL))
...
func (s *Sections) Post(post *posts.PostResponse) template.HTML {
	if post == nil {
		return template.HTML("")
	}
...
func (s *Sections) Social(socialResponse *links.LinksResponse) template.HTML {
	if socialResponse == nil {
		return template.HTML("<h1>Social</h1><p><ul></ul></p>")
	}

	var items []string

	for _, item := range socialResponse.Data {
...
func (s *Sections) Contact(profileResponse *profile.ProfileResponse) template.HTML {
	if profileResponse == nil {
		return s.contactFallback()
	}

	email := template.HTMLEscapeString(strings.TrimSpace(profileResponse.Data.Email))
	if email == "" {
		return s.contactFallback()
	}
...
func (s *Sections) Recommendations(recs *recommendations.RecommendationsResponse) template.HTML {
	if recs == nil {
...
func (s *Sections) Experience(exp *experience.ExperienceResponse) template.HTML {
	if exp == nil {
...
func (s *Sections) Education(edu *education.EducationResponse) template.HTML {
	if edu == nil {
