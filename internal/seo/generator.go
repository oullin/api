...
func (g *Generator) GenerateIndex() error {
	var err error
	var talksResponse *talks.TalksResponse
	var profileResponse *profile.ProfileResponse
	var projectsResponse *projects.ProjectsResponse

	cli.Cyanln("Fetching profile for home page")
	if profileResponse, err = g.Client.GetProfile(); err != nil {
		return fmt.Errorf("home: fetching profile: %w", err)
	}

	cli.Cyanln("Fetching talks for home page")
	if talksResponse, err = g.Client.GetTalks(); err != nil {
		return fmt.Errorf("home: fetching talks: %w", err)
	}

	cli.Cyanln("Fetching projects for home page")
	if projectsResponse, err = g.Client.GetProjects(); err != nil {
	...
	html = append(html, sections.Talks(talksResponse))
	html = append(html, sections.Skills(profileResponse))
	html = append(html, sections.Projects(projectsResponse))
...
func (g *Generator) generatePostSEO(sections Sections, post database.Post) error {
	...
	response := posts.GetPostsResponse(post)
...
func (g *Generator) FilterBrandLinks(social *links.LinksResponse) *links.LinksResponse {
	if social == nil {
		return nil
	}

	brand := strings.ToLower(g.Web.Brand.Name)
	filtered := make([]links.LinksData, 0, len(social.Data))
	...
	return &links.LinksResponse{
		Version: social.Version,
		Data:    filtered,
	}
}

func (g *Generator) BuildForPost(post posts.PostResponse, body []template.HTML) (TemplateData, error) {
