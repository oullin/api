...
func TestEnrichResponse_RejectsInvalidData(t *testing.T) {
	cases := []struct {
		...
		{
			name:      "nil sort",
			data:      ProjectsData{UUID: "bad-project", Sort: nil, PublishedAt: "2026-03-10"},
			wantMatch: "no sort value",
		},
		{
			name:      "zero sort",
			data:      ProjectsData{UUID: "zero-sort", Sort: intPtr(0), PublishedAt: "2026-03-10"},
			wantMatch: "invalid sort value",
		},
		{
			name:      "negative sort",
			data:      ProjectsData{UUID: "negative-sort", Sort: intPtr(-1), PublishedAt: "2026-03-10"},
			wantMatch: "invalid sort value",
		},
		{
			name:      "empty published_at",
			data:      ProjectsData{UUID: "no-date", Sort: intPtr(1), PublishedAt: ""},
			wantMatch: "empty published_at",
		},
		{
			name:      "whitespace-only published_at",
			data:      ProjectsData{UUID: "space-date", Sort: intPtr(1), PublishedAt: "   "},
			wantMatch: "empty published_at",
		},
	}
