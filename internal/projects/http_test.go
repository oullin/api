...
func TestProjectsHandler_SortsAndPaginates(t *testing.T) {
	fixture := writeProjectsFixture(t, ProjectsResponse{
		Version: "1.0.0",
		Data: []ProjectsData{
	...
	h := NewProjectsHandlerWithCache(fixture, true)
...
func TestProjectsHandler_NoStoreWhenCacheDisabled(t *testing.T) {
	...
	h := NewProjectsHandlerWithCache(fixture, false)
	req := httptest.NewRequest(http.MethodGet, "/projects", nil)
	rec := httptest.NewRecorder()
...
func writeProjectsFixture(t *testing.T, data ProjectsResponse) string {
	t.Helper()
